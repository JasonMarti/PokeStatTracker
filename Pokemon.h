...
class Pokemon {
  public:
    ...
    Pokemon(string csv);
    // new
    Pokemon(string name, int hp_base, int att_base, int def_base, int spec_att_base, int spec_def_base, int speed_base,
            string nature, int level);
    string name;
    string nature;
    int currentLevel;
    Stat hp;
  ...
  private:
    ...
    void calc_current_stat(Stat &stat, int level, float nature);
    void calc_current_hp(Stat &hp, int level);
