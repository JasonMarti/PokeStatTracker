...
void updateDisplay(gpointer ptr) {
  ...
  GtkWidget *comboBoxText =
      (GtkWidget *)gtk_builder_get_object(builder, "profiles_combo_box");
  Pokemon tempPoke = pokemans.at(
      gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(comboBoxText)));

  // set name
  GtkWidget *singleWorker =
  ...
  output =
      to_string(tempPoke.att.bottomIV) + "-" + to_string(tempPoke.att.bottomIV);
  gtk_label_set_text(GTK_LABEL(statWorker), output.c_str());
  statWorker = (GtkWidget *)gtk_builder_get_object(builder, "def_iv");
  output =
      to_string(tempPoke.def.bottomIV) + "-" + to_string(tempPoke.def.bottomIV);
  gtk_label_set_text(GTK_LABEL(statWorker), output.c_str());
  statWorker = (GtkWidget *)gtk_builder_get_object(builder, "spec_att_iv");
  output = to_string(tempPoke.spec_attack.bottomIV) + "-" +
           to_string(tempPoke.spec_attack.bottomIV);
  gtk_label_set_text(GTK_LABEL(statWorker), output.c_str());
  statWorker = (GtkWidget *)gtk_builder_get_object(builder, "spec_def_iv");
  output = to_string(tempPoke.spec_def.bottomIV) + "-" +
           to_string(tempPoke.spec_def.bottomIV);
  gtk_label_set_text(GTK_LABEL(statWorker), output.c_str());
  statWorker = (GtkWidget *)gtk_builder_get_object(builder, "speed_iv");
  output = to_string(tempPoke.speed.bottomIV) + "-" +
           to_string(tempPoke.speed.bottomIV);
  gtk_label_set_text(GTK_LABEL(statWorker), output.c_str());
  ...
  statWorker =
      (GtkWidget *)gtk_builder_get_object(builder, "spec_att_ev_entry");
  gtk_spin_button_set_value(GTK_SPIN_BUTTON(statWorker),
                            tempPoke.spec_attack.EV);
  statWorker =
      (GtkWidget *)gtk_builder_get_object(builder, "spec_def_ev_entry");
...
void level_change_handler(GtkWidget *wid, gpointer ptr) {
  ...
  GtkWidget *worker =
      (GtkWidget *)gtk_builder_get_object(builder, "profiles_combo_box");
  Pokemon currentProfile = pokemans.at(
      gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(worker)));
...
void load_button_handler(GtkWidget *wid, gpointer ptr) {
  ...
  GtkWidget *load_button =
      (GtkWidget *)gtk_builder_get_object(builder, "load_button");
  ifstream fin;
  const char *loadFile;
  loadFile =
      gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(load_button));
  char *output;
  string temp;
  string str;
...
void new_pokemon_ok_handler(GtkWidget *wid, gpointer ptr) {
  ...
  // name
  GtkWidget *nameField =
      (GtkWidget *)gtk_builder_get_object(builder, "new_pokemon_name_entry");
  string name = gtk_entry_get_text(GTK_ENTRY(nameField));
  // nature
  GtkWidget *natureField =
      (GtkWidget *)gtk_builder_get_object(builder, "new_natures");
  string nature =
      gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(natureField));
  // stats
  GtkWidget *hpField =
      (GtkWidget *)gtk_builder_get_object(builder, "new_pokemon_hp_entry");
  GtkWidget *attField =
      (GtkWidget *)gtk_builder_get_object(builder, "new_pokemon_att_entry");
  GtkWidget *defField =
      (GtkWidget *)gtk_builder_get_object(builder, "new_pokemon_defense_entry");
  GtkWidget *specAttField = (GtkWidget *)gtk_builder_get_object(
      builder, "new_pokemon_spec_att_entry");
  GtkWidget *specDefField = (GtkWidget *)gtk_builder_get_object(
      builder, "new_pokemon_spec_def_entry");
  GtkWidget *speedField =
      (GtkWidget *)gtk_builder_get_object(builder, "new_pokemon_speed_entry");
  // level
  GtkWidget *levelField =
      (GtkWidget *)gtk_builder_get_object(builder, "new_pokemon_level_entry");
  ...
  pokemans.insert(pair<string, Pokemon>(
      name, Pokemon(name, atoi(gtk_entry_get_text(GTK_ENTRY(hpField))),
                    ...
                    atoi(gtk_entry_get_text(GTK_ENTRY(speedField))), nature,
